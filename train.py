...
optimizer = optim.Adam(net_DF.parameters(), lr=cfgs.learning_rate)
    ...
    for batch_idx, batch_data_label in enumerate(TRAIN_DATALOADER):
        for key in batch_data_label:
            if 'list' in key:
                ...
                batch_data_label[key] = batch_data_label[key].to(device)  # (2, 720, 1280)
...
def train(start_epoch):
    ...
    for epoch in range(start_epoch, cfgs.max_epoch):
        ...
        save_dict = {'epoch': epoch + 1, 'optimizer_state_dict': optimizer.state_dict(),
                     'model_state_dict': net_DF.state_dict()}
